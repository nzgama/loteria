...
if (isset($_POST['add_cat'])) {
  $req_field = array('vendedores-name');
  validate_fields($req_field);
  $cat_name = remove_junk($db->escape($_POST['vendedores-name']));
  if (empty($errors)) {
    $sql  = "INSERT INTO categories (name)";
    $sql .= " VALUES ('{$cat_name}')";
    if ($db->query($sql)) {
      $session->msg("s", "Categoría agregada exitosamente.");
      redirect('vendedores.php', false);
    } else {
      ...
      <div class="panel-body">
        <form method="post" action="vendedores.php">
          <div class="form-group">
            <input type="text" class="form-control" name="vendedores-name" placeholder="Nombre del Vendedor" required>
          </div>
          <button type="submit" name="add_cat" class="btn btn-primary">Agregar Vendedor</button>
        </form>
      </div>
    </div>
  </div>
